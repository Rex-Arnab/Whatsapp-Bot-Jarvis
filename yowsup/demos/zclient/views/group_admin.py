...
class GroupAdminViews():
    ...
    def __init__(self, interface_layer):
        ...
        self.routes = [
            ...
            ("subject (?P<status>[^$]+)$", self.group_subject),
            #("^/ban\s(?P<phone_number>[0-9]{8,14})\s*$", self.ban),
        ]

    def group_subject(self, group_jid, message, match):
        if self._is_authorized(message):
            subject = match.group("status")
            entity = SubjectGroupsIqProtocolEntity(message.getFrom(), subject)
    ...
    def kick(self, message, match):
        ...
        """
        if self._is_authorized(message):
            if message.getFrom() != "919851478875@s.whatsapp.net":
