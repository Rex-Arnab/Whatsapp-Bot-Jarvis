"""
    ...
    youtube urls
"""
from yowsup.demos.zclient.utils.media_sender import ImageSender, VideoSender, YoutubeSender, UrlPrintSender, EspeakTtsSender
...
class GoogleViews():
    def __init__(self, interface_layer):
        ...
        self.google_tts_sender = EspeakTtsSender(interface_layer)
        self.routes = [
    ...
    def google_search(self, message, match):
        ...
        req = requests.get("https://www.googleapis.com/customsearch/v1?key=AIzaSyAdEGAoIBMiNU6PyFyMykvIrNO957wQovM&cx=017576662512468239146:omuauf_lfve&q=%s" % match.group("term"))
        page_url = urllib.parse.unquote(req.json()["responseData"]["results"][0]["url"])
    ...
    def google_image_search(self, message, match):
        ...
        image_url = urllib.parse.unquote(req.json()["responseData"]["results"][0]["url"])
