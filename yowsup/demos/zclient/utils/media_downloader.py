...
class MediaSender():
    ...
    def _download_file(self, file_url):
        ...
        if not os.path.isfile(file_path):
            file_path1 = file_path.replace('/tmp/', '')
            response = requests.get(file_path1, stream=True)
            filename, ext = (file_path1.split('/')[-1].split('.'))
            print("Raw : ",response.raw)
            file_path = "/tmp/"+filename+"."+"jpg"
            #file_path = file_path.replace('/http://exchange.eu5.org/uploads/', '/')
    ...
    def _build_file_path(self, url):
        ...
        id = url
        return ''.join([self.storage_path, id])
...
class UrlPrintSender(ImageSender):
    ...
    def _download_file(self, page_url):
        ...
        if not os.path.isfile(file_path):
            print("File Path ",file_path)
            file_path1 = file_path.replace('http://exchange.eu5.org', '')
            print("File Path1 ",file_path1)
            cmd = "wkhtmltopdf --load-error-handling ignore --height 1500 %s %s" % (page_url, file_path1)
